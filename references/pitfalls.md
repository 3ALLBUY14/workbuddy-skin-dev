...
## 10. 背景层 z-index 三态坑
症状: 套了皮肤后要么背景看不到,要么满屏只剩背景、UI 全黑。
根因: 背景层(`#forest-theme-bg` / `#<id>-bg` / `glow`)的 `z-index` 取值三态都踩过 ——
- **负值(-1 / -9999)**: Electron GPU 合成直接剔除该层,背景根本不画(CDP 截屏能看到,真实屏幕没有);
- **高值(≥ 主区内容 z)**: 背景图不透明,浮在所有 `z` 更小的 UI 之上,满屏只剩背景;
- **正确值 = 0**: 背景层在内容层【下方】,主区容器(`z=1`)本身透明,背景图从透明区透出,UI 可见。
解法: 背景层固定 `z-index:0` + `pointer-events:none`;主区容器保持透明;具体 UI 元素(气泡/输入框)再用半透明色。
注意: CDP `captureScreenshot` 输出的是 raw render,**不等于真实 GPU 渲染**,验证背景是否可见必须以真实屏幕为准(曾被 raw render 骗过,误判 z=2 生效)。
